...
void spkmeans(char *filename, char *goal, int k, int source) /*  should change */
{
    int dim, n, longest;
    ...
    printf("n is: %d\n",n);
    points = get_mat_from_file(fp,n,dim);
    ...
    else if (0 == strcmp(goal, "spk"))
    {
        ...
        /*printeigen(sortedEigen,n);*/
        if (k == 0)
        {
            k = eigengap(sortedEigen, n);
        }
        printf("the calculated k is %d ",k);
        U = make_me_a_k_matrix(n, k, sortedEigen);
        printf("\n **finish make_me_kmatrix\n result:\n");
        print_matrix(U,n,k);
